...
withTempFd :: (Fd -> IO a) -> IO a
withTempFd action =
  withSystemTempDirectory "filepage-spec" $ \tmpDir ->
    bracket
      (PosixIO.openFd (tmpDir ++ "/test.db") PosixIO.ReadWrite PosixIO.defaultFileFlags
  ...
  describe "read/write" $ do
    it "should handle page IO operations" $ do
      withTempFd $ \fd -> do
        ...
        _ <- FilePage.write fd (ix, pageSize) testData
        result <- FilePage.read fd (ix, pageSize)
    ...
    it "should handle multiple pages" $ do
      withTempFd $ \fd -> do
        ...
        let pageSize = FilePage.Size  8
        _ <- FilePage.write fd (FilePage.Index 0, pageSize) page0Data
        _ <- FilePage.write fd (FilePage.Index 1, pageSize) page1Data
        
        result0 <- FilePage.read fd (FilePage.Index 0, pageSize)
        result1 <- FilePage.read fd (FilePage.Index 1, pageSize)
    ...
    it "should handle invalid page indices" $ do
      withTempFd $ \fd -> do
        result <- FilePage.read fd (FilePage.Index (-1), FilePage.Size 8192)
    ...
    it "should handle different page sizes" $ do
      withTempFd $ \fd -> do
        ...
        _ <- FilePage.write fd (FilePage.Index 0, pageSize) testData
        result <- FilePage.read fd (FilePage.Index 0, pageSize)
        result `shouldBe` Right testData
